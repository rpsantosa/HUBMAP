...
#model<-load_model_hdf5(file.path(MODEL,"m8774.hdf5"), custom_objects = c("bce_dice_loss"= bce_dice_loss, "dice"=dice))
...
xm <- list.files(MASK,full.names = TRUE)
xm<- xm[sapply(xm, file.size) >1500]
exmt<- xm %>% map( . %>% gsub(pattern = file.path(MASK,"mask"), replacement = ""))
ttrain<-   paste0(file.path(TRAIN,"image"),exmt) %>% unlist
imagesm <- tibble(
  img = ttrain,
...
csplit<- initial_split(imagest, prop = 8/10,strata = glo)
...
##########################################################################################
# base_model<- model <- application_resnet50(weights = NULL,
...
hist<- model %>% 
  ...
  save_model_hdf5(model, file.path(MODEL,"m83_80_20.hdf5") ) 
...
batch <- cd %>% as_iterator() %>% iter_next()
predictions <- predict(model, batch[[1]])
...
makeimg<-function(i,j,im=im){
  ...
  shape<-dataset %>% subset(image_file == paste0(ffnames[k],".tiff" )) %>% dplyr::select(height_pixels,width_pixels) %>% unlist %>% as.vector
  nl<-c( seq(0,shape[1]-512, 512), shape[1]-512)
  nc<-c( seq(0,shape[2]-512, 512) , shape[2]-512)
  nn<-expand.grid(nl,nc)
  # walk2(nn[,1],nn[,2],makePng,im=im,mask=mask)
